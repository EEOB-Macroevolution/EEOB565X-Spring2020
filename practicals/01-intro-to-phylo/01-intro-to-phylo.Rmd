...
You will notice that the tree does not have meaningful branch lenghts. We can represent the branch lengths in units of the number of changes (under parsimony) using the `acctran()` function. 
...
Now we can optimize the tree topology under parsimony using the `optim.parsimony()` function. This performs a series of tree rearrangments under NNI. 
...
What is different about your starting tree and the optimized tree?

### Maximum Likelihood

Compute the likelihood of the parsimony tree and parsimony branch lenghts. For this we use the `pml()` function of _phangorn_. By default, this approach computes the likelihood under the Jukes-Cantor (1969) model:
...
We do not want to rely on parsimony for the tree topology and branch lengths, but this tree provides a reasonable starting place. We can use the `optim.pml()` function to optimize the tree and model parameters under JC69. This will perform NNI tree rearrangments to identify the topology that maximizes the likelihood.
...
If we want to optimize the tree using a different model like e.g., GTR, we can use the `pml` object as an argument and specify the `GTR` model. Perhapse we would also like to change to `SPR` for tree rearrangements. 

```{r}
fitGTR <- optim.pml(fitJC, model="GTR", rearrangement = "SPR")
GTR_tree_rooted <- root(fitGTR$tree,outgroup)
plot(GTR_tree_rooted)
```


View the deatils of the `fitGTR` object. What is the loglikelihood?

```{r}
fitGTR
```

How are the tw trees different? 

...
create_TN93_Q_matrix = function(pi, alpha1, alpha2, beta) {
    ...
    piA = pi[3]
    piG = pi[4]
    Q = matrix(nrow = 4, ncol = 4, byrow = TRUE,
               data = c(c(0, alpha1 * piC, beta * piA, beta * piG),
                        c(alpha1 * piT, 0, beta * piA, beta * piG),
                        c(beta * piT, beta * piC, 0, alpha2 * piG),
                        c(beta * piT, beta * piC, alpha2 * piA, 0)))
    diag(Q) = -rowSums(Q)
...
create_K80_Q_matrix = function(alpha, beta) {
    # Create the K80 transition rate matrix Q as specified in the assignment.
    # The nucleotide order should be (T, C, A, G).
    #   alpha: rate coefficient for transitions
    #   beta: rate coefficient for transversions

    Q = matrix(nrow = 4, ncol = 4, byrow = TRUE,
               data = c(c(0, alpha, beta, beta),
                        c(alpha, 0, beta, beta),
                        c(beta, beta, 0, alpha),
                        c(beta, beta, alpha, 0)))
    diag(Q) = -rowSums(Q)
...
calculate_likelihood_from_subtree_likelihoods = function(N, Q,
    ...
    # Calculate the likelihood of each nucleotide at each site from the two subtree likelihoods.
    ...
    subtree1_P = expm(Q * subtree1_branch_length)
    subtree2_P = expm(Q * subtree2_branch_length)
    for (site in 1:N) {
        for (nucleotide_parent in 1:4) {
            likelihood1 = sum(subtree1_P[nucleotide_parent, ] * subtree1_likelihood[site, ])
            likelihood2 = sum(subtree2_P[nucleotide_parent, ] * subtree2_likelihood[site, ])
            likelihood_per_site[site, nucleotide_parent] = likelihood1 * likelihood2
        }
    }

    # Return the per site nucleotide likelihoods.
...
get_likelihood_from_sequence = function(N, sequence) {
    # Get the matrix of likelihoods of each nucleotide at each site at the tip node given the sequence.
    ...
    for (site in 1:N) {
        likelihood = rep(0, 4)
        likelihood[sequence[site]] = 1
        likelihood_per_site[site,] = likelihood
    }
...
get_subtree_likelihood = function(node, tree, sequences, Q) {
    ...
    #   tree: the tree encoded in an object of class phylo
    #   sequences: the sequence alignment at the tips of the tree
    #   Q: the substitution rate matrix

    N = nchar(sequences[[1]])

    if (node <= length(tree$tip.label)) {
        # Compute the likelihood for each site at the tip of the tree

        sequence = get_sequence_at_tip_node(node, tree, sequences)
        likelihood_per_site = get_likelihood_from_sequence(N, sequence)
    } else {
        # Compute the likelihood for each site at the internal node of the tree

        subtrees = get_node_children(node, tree)
        subtree1_likelihood = get_subtree_likelihood(subtrees$child1, tree, sequences, Q)
        subtree2_likelihood = get_subtree_likelihood(subtrees$child2, tree, sequences, Q)
        likelihood_per_site = calculate_likelihood_from_subtree_likelihoods(N, Q,
                                                                            subtree1_likelihood,
                                                                            subtrees$branch_length1,
                                                                            subtree2_likelihood,
                                                                            subtrees$branch_length2)
    }

    # Return the per site nucleotide likelihoods.
    #   likelihood_per_site: an N by 4 matrix representing the nucleotide likelihoods per site in the alignment
    return(likelihood_per_site)
}
...
Felstensteins_pruning_loglikelihood = function(pi, alpha1, alpha2, beta, newick_tree, sequences) {
    ...
    Q_matrix = create_TN93_Q_matrix(pi, alpha1, alpha2, beta)
    
    # Compute the likelihoods per site of the tree starting from the root
    root = length(tree$tip.label) + 1
    likelihood_per_site = get_subtree_likelihood(root, tree, sequences, Q_matrix)
    
    # Sum up the log likelihoods of each site
    loglikelihood = 0
    for (site in 1:N) {
        site_likelihood = sum(pi * likelihood_per_site[site, ])
        loglikelihood = loglikelihood + log(site_likelihood)
    }
...
test_tree_loglikelihood = function() {
    ...
    if (isTRUE(comparison)) {
        ...
        print(sprintf("Difference between expectation and result is %.*f", 10, expected_result - result))
    }
}
